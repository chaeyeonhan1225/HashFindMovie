    ...
    <v-container>
      <v-row>
        <div style="margin-right:20px">
          <v-icon
            style="position: relative; top: 40px; left: 100px;z-index: 2;"
            large
            color="black"
            >mdi-camera</v-icon
          >
          <v-avatar
            :color="me ? me.color : 'indigo'"
        ...
        <div v-if="me" style="min-width: 500px">
          <h1>
            <span class="mark_pen-yellow" v-if="me !== null">{{ me.nick }}</span
            >님
          </h1>
          <h3 style="display:inline;margin-right:5px" v-if="me.info === null">
            한줄 소개를 작성해주세요!
          </h3>
          <h3 style="display:inline;margin-right:5px" v-else>{{ me.info }}</h3>
          <v-icon style="bottom: 2px" color="gray" @click="onToggle()">{{
            toggleIcon
          }}</v-icon>
          <div v-if="toggleOn">
        ...
        <v-card outlined>
          <div style="padding:15px">
            <span>
              ...
              <router-link to="/profile/liked" style="margin-left: 10px"
                >더보기</router-link
              >
            </span>
            <LikedPreview :liked="movies" />
            <h2 style="display:inline">나의 한줄 평</h2>
            <span style="margin-left:5px" v-if="me.comments">{{
              me.comments.length
            }}</span>
            <span class="ml-2"
              ><v-btn icon @click="loadComments()"
...
export default {
  ...
  created() {
    return Promise.all([this.loadComments(), this.loadLiked()]);
  },
  data() {
    return {
      toggleOn: false,
  ...
  methods: {
    loadComments() {
      this.$store
        .dispatch("userStore/loadComments", {
          offset: 0,
          limit: 5,
        })
        .then((result) => {
    ...
    onSubmitForm() {
      if (this.$refs.form.validate()) {
        ...
        this.$store
          .dispatch("userStore/changeUserInfo", {
